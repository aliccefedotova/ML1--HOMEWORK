...
#метод __call__ поддерживает все схемы передачи аргументов. Все что передается экземпляру, передается этому методу наряду с обычным аргументом self, в котором передается сам экземпляр.
    def __call__(self):
        """
...
class BaseDescent:
    ...
    """
#магический метод __init__ систематически при создании объекта создает ему атрибуты т.е. вызывать данный метод не нужно
#т.к. он сам запускается при вызове класса (внутри метода атрибуты экземпляра ссылаются пр помощи ключа self
...
#cпросить у Фила про подчет лоса - return 2 / l * (x.T @ (x @ self.w) - x.T @ y) как правильно
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        # TODO: implement calculating gradient function
        indexes = np.random.randint(0, x.shape[0], size = self.batch_size)
        #syper() обеспечивает доступ к оригиналам наследованных методов
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        w_prev = self.w.copy()
        w_next = w_prev - (self.lr() / (np.sqrt(v_) + self.eps)) * m_
        self.w = w_next
...
class BaseDescentReg(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        # помним, что свободный коэффициент w_0 не включаем в регуляризатор
        # l2_gradient[-1] = 0
        l2_gradient = self.w[1:] * self.mu
